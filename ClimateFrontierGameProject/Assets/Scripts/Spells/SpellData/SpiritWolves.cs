...
public class SpiritWolves : MonoBehaviour, IPoolable, ISpell
{
    [Header("AOE Settings")]
    public float speed = 5f;           // Speed at which the AOE moves forward
    ...
    private void Awake()
    {
        ...
        if (aoeCollider == null)
        {
            Debug.LogError("Spirit Wolves script requires a Collider component on the same GameObject.");
        }
        else if (!aoeCollider.isTrigger)
        {
            Debug.LogWarning("Spirit Wolves Collider should be set as Trigger.");
    ...
    public void Initialize(SpellData spellData, BasePlayer player, Transform target)
    {
        ...
        Debug.Log($"Spirit Wolves Initialized with Damage: {spellDamage}, Range: {range}, Speed: {speed}, Active Duration: {activeDuration}, Pool Tag: {poolTag}");
    }
    ...
    private void Update()
    {
        ...
        {
            // If no target is assigned, deactivate the wolf
            Debug.LogWarning($"{gameObject.name}: No target assigned. Deactivating.");
    ...
    private void DeactivateAndReturnToPool()
    {
        ...
        {
            ObjectPooler.Instance.ReturnToPool("Wolf", gameObject);
        }
        else
        {
